...
(defun lat-lon-to-utm (lat lon &key (ellipsoid "WGS84") (zone nil))
  ...
  (let* ((lat-rad (deg2rad lat))
         ...
         (k3 (realpart (* (* k0 nu (sin lat-rad) (expt (cos lat-rad) 3.0d0) (/ 1.0d0 24.0d0))
                (+ 5.0d0 (- (expt (tan lat-rad) 2.0d0)) (* 9.0d0 e-prime-squared (expt (cos lat-rad) 2.0d0)) (* 4.0d0 (expt e-squared 2.0d0) (expt (cos lat-rad) 4.0d0))))))
         (k4 (* k0 nu (cos lat-rad)))
         (k5 (realpart (* (* k0 nu (expt (cos lat-rad) 3.0d0) (/ 1.0d0 6.0d0))
                (+ 1.0d0 (- (expt (tan lat-rad) 2.0d0)) (* e-prime-squared (expt (cos lat-rad) 2.0d0))))))
         (easting (realpart (+ E0 (* k4 p) (* k5 (expt p 3.0d0)))))
         (northing (realpart (+ k1 (* k2 (expt p 2.0d0)) (* k3 (expt p 4.0d0))))))
    (declare (type double-float lat-rad lon-rad a b f e-squared e-prime-squared
                   long0 nu p M K1 K2 K3 k4 k5 easting northing)
             (type fixnum nzone))
    (list easting
          (if (< lat 0 ) (+ N0 northing) northing)
          nzone)))
